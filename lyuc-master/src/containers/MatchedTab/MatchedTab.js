...
class MatchedTab extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            matchedList: []
            ...
            snapshot.forEach((child)=>{
                ...
                if(matched === 0) {
                    ...
                    ref.once('value').then((data) => {
                        ...
                        this.setState({matchedList: [...this.state.matchedList, {firstName: firstName, lastName: lastName, avatarUri: avatarUri, uid: uid}]});
                    })
                }
            });
        })
    ...
    goToProfile(index) {
        AsyncStorage.setItem('selectedUid', this.state.matchedList[index].uid)
        AsyncStorage.setItem('UserProfileType', 'matched');
        Actions.Userprofile();
    }
